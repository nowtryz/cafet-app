...
class DataFetcher extends DatabaseConnection
{
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'ID id, ' . 'Email email, ' . 'Pseudo alias, ' . 'Nom fname, ' . 'Prenom sname, ' . 'adherent member, ' . 'Credit balance, ' . 'Annee regyear ' . 'FROM ' . self::USERS . ' u ' . 'WHERE u.SU = 0 ' . 'ORDER BY u.Prenom');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'ID id, ' . 'Email email, ' . 'Pseudo alias, ' . 'Nom fname, ' . 'Prenom sname, ' . 'adherent member, ' . 'Credit balance, ' . 'Annee regyear ' . 'FROM ' . self::USERS . ' u ' . 'WHERE id = :id ');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'id id, ' . 'user_balance balance, ' . 'amount amount, ' . 'details details, ' . 'DATE_FORMAT(date, "%H") hour, ' . 'DATE_FORMAT(date, "%i") mins, ' . 'DATE_FORMAT(date, "%s") secs, ' . 'DATE_FORMAT(date, "%d") day, ' . 'DATE_FORMAT(date, "%c") month, ' . 'DATE_FORMAT(date, "%Y") year ' . 'FROM ' . self::RELOADS . ' ' . 'WHERE user_id = :id ' . 'ORDER BY date DESC');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'e.id id, ' . 'e.user_balance balance, ' . '(SELECT SUM(edit.price * f.quantity) ' . 'FROM ' . self::FORMULAS_BOUGHT . ' f ' . 'LEFT JOIN ' . self::FORMULAS_EDITS . ' edit ' . 'ON f.edit_id = edit.id ' . 'WHERE f.expense_id = e.id ' . 'GROUP BY f.expense_id) ftotal, ' . '(SELECT SUM(edit.price * p.quantity) ' . 'FROM ' . self::PRODUCTS_BOUGHT . ' p ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' edit ' . 'ON p.edit_id = edit.id ' . 'WHERE p.expense_id = e.id ' . 'GROUP BY p.expense_id) ptotal, ' . 'DATE_FORMAT(e.date, "%H") hour, ' . 'DATE_FORMAT(e.date, "%i") mins, ' . 'DATE_FORMAT(e.date, "%s") secs, ' . 'DATE_FORMAT(e.date, "%d") day, ' . 'DATE_FORMAT(e.date, "%c") month, ' . 'DATE_FORMAT(e.date, "%Y") year ' . 'FROM ' . self::EXPENSES . ' e ' . 'WHERE e.user_id = :id ' . 'ORDER BY e.date DESC');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'e.id id, ' . 'e.user_balance balance, ' . '(SELECT SUM(edit.price * f.quantity) ' . 'FROM ' . self::FORMULAS_BOUGHT . ' f ' . 'LEFT JOIN ' . self::FORMULAS_EDITS . ' edit ' . 'ON f.edit_id = edit.id ' . 'WHERE f.expense_id = e.id ' . 'GROUP BY f.expense_id) ftotal, ' . '(SELECT SUM(edit.price * p.quantity) ' . 'FROM ' . self::PRODUCTS_BOUGHT . ' p ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' edit ' . 'ON p.edit_id = edit.id ' . 'WHERE p.expense_id = e.id ' . 'GROUP BY p.expense_id) ptotal, ' . 'DATE_FORMAT(e.date, "%H") hour, ' . 'DATE_FORMAT(e.date, "%i") mins, ' . 'DATE_FORMAT(e.date, "%s") secs, ' . 'DATE_FORMAT(e.date, "%d") day, ' . 'DATE_FORMAT(e.date, "%c") month, ' . 'DATE_FORMAT(e.date, "%Y") year ' . 'FROM ' . self::EXPENSES . ' e ' . 'WHERE e.user_id = :id ' . 'AND e.date > (SELECT MAX(date) FROM ' . self::RELOADS . ' WHERE user_id = :id2) ' . 'ORDER BY e.date DESC');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'e.id id, ' . 'e.user_balance balance, ' . '(SELECT SUM(edit.price * f.quantity) ' . 'FROM ' . self::FORMULAS_BOUGHT . ' f ' . 'LEFT JOIN ' . self::FORMULAS_EDITS . ' edit ' . 'ON f.edit_id = edit.id ' . 'WHERE f.expense_id = e.id ' . 'GROUP BY f.expense_id) ftotal, ' . '(SELECT SUM(edit.price * p.quantity) ' . 'FROM ' . self::PRODUCTS_BOUGHT . ' p ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' edit ' . 'ON p.edit_id = edit.id ' . 'WHERE p.expense_id = e.id ' . 'GROUP BY p.expense_id) ptotal, ' . 'DATE_FORMAT(e.date, "%H") hour, ' . 'DATE_FORMAT(e.date, "%i") mins, ' . 'DATE_FORMAT(e.date, "%s") secs, ' . 'DATE_FORMAT(e.date, "%d") day, ' . 'DATE_FORMAT(e.date, "%c") month, ' . 'DATE_FORMAT(e.date, "%Y") year ' . 'FROM ' . self::EXPENSES . ' e ' . 'WHERE e.id = :id');
    ...
    public final function getExpenseDetails(int $expense_id): array
    {
        ...
        $stmt = $this->connection->prepare('SELECT ' . 'b.id id, ' . 'e.name name, ' . 'e.price price, ' . 'b.formula_id fid, ' . 'b.quantity quantity, ' . 'b.user_id client_id, ' . 'DATE_FORMAT(b.date, "%H") hour, ' . 'DATE_FORMAT(b.date, "%i") mins, ' . 'DATE_FORMAT(b.date, "%s") secs, ' . 'DATE_FORMAT(b.date, "%d") day, ' . 'DATE_FORMAT(b.date, "%c") month, ' . 'DATE_FORMAT(b.date, "%Y") year ' . 'FROM ' . self::FORMULAS_BOUGHT . ' b ' . 'LEFT JOIN ' . self::FORMULAS_EDITS . ' e ' . 'ON b.edit_id = e.id ' . 'WHERE b.expense_id = :id');
        ...
        $stmt = $this->connection->prepare('SELECT ' . 'b.id id, ' . 'e.name name, ' . 'e.price price, ' . 'b.product_id pid, ' . 'b.quantity quantity, ' . 'b.user_id client_id, ' . 'DATE_FORMAT(b.date, "%H") hour, ' . 'DATE_FORMAT(b.date, "%i") mins, ' . 'DATE_FORMAT(b.date, "%s") secs, ' . 'DATE_FORMAT(b.date, "%d") day, ' . 'DATE_FORMAT(b.date, "%c") month, ' . 'DATE_FORMAT(b.date, "%Y") year ' . 'FROM ' . self::PRODUCTS_BOUGHT . ' b ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' e ' . 'ON b.edit_id = e.id ' . 'WHERE b.expense_id = :id');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'id, ' . 'name, ' . 'display_name dname ' . 'FROM ' . self::PRODUCTS_GROUPS . ' ' . 'ORDER BY id');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'id, ' . 'name, ' . 'display_name dname ' . 'FROM ' . self::PRODUCTS_GROUPS . ' ' . 'WHERE id = :id ' . 'ORDER BY id');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'p.id id, ' . 'e.name name, ' . 'e.price price, ' . 'p.image image, ' . 'p.viewable viewable, ' . 'DATE_FORMAT(e.edit, "%H") hour, ' . 'DATE_FORMAT(e.edit, "%i") mins, ' . 'DATE_FORMAT(e.edit, "%s") secs, ' . 'DATE_FORMAT(e.edit, "%d") day, ' . 'DATE_FORMAT(e.edit, "%c") month, ' . 'DATE_FORMAT(e.edit, "%Y") year ' . 'FROM ' . self::PRODUCTS . ' p ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' e ' . 'ON p.last_edit = e.id ' . 'WHERE p.product_group = :group' . (! $show_hiddens ? ' AND p.viewable = 1' : ''));
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'user_id client_id ' . 'FROM ' . self::FORMULAS_BOUGHT . ' ' . 'WHERE id = :id');
        ...
        $stmt = $this->connection->prepare('SELECT ' . 'b.product_id product_id, ' . 'e.name name, ' . 'DATE_FORMAT(b.date, "%H") hour, ' . 'DATE_FORMAT(b.date, "%i") mins, ' . 'DATE_FORMAT(b.date, "%s") secs, ' . 'DATE_FORMAT(b.date, "%d") day, ' . 'DATE_FORMAT(b.date, "%c") month, ' . 'DATE_FORMAT(b.date, "%Y") year ' . 'FROM ' . self::FORMULAS_BOUGHT_PRODUCTS . ' b ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' e ' . 'ON b.product_edit = e.id ' . 'WHERE b.transaction_id = :fb_id');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'p.id id, ' . 'e.name name, ' . 'e.price price, ' . 'p.image image, ' . 'p.product_group group_id, ' . 'p.viewable viewable, ' . 'DATE_FORMAT(e.edit, "%H") hour, ' . 'DATE_FORMAT(e.edit, "%i") mins, ' . 'DATE_FORMAT(e.edit, "%s") secs, ' . 'DATE_FORMAT(e.edit, "%d") day, ' . 'DATE_FORMAT(e.edit, "%c") month, ' . 'DATE_FORMAT(e.edit, "%Y") year ' . 'FROM ' . self::PRODUCTS . ' p ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' e ' . 'ON p.last_edit = e.id ' . 'WHERE p.id = :id');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'f.id id, ' . 'e.name name, ' . 'f.image image, ' . 'e.price price, ' . 'f.viewable viewable, ' . 'DATE_FORMAT(e.edit, "%H") hour, ' . 'DATE_FORMAT(e.edit, "%i") mins, ' . 'DATE_FORMAT(e.edit, "%s") secs, ' . 'DATE_FORMAT(e.edit, "%d") day, ' . 'DATE_FORMAT(e.edit, "%c") month, ' . 'DATE_FORMAT(e.edit, "%Y") year ' . 'FROM ' . self::FORMULAS . ' f ' . 'LEFT JOIN ' . self::FORMULAS_EDITS . ' e ' . 'ON f.last_edit = e.id' . (! $show_hiddens ? ' WHERE f.viewable = 1' : ''));
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'id, ' . 'name ' . 'FROM ' . self::FORMULAS_CHOICES . ' ' . 'WHERE formula = :id');
        ...
        $stmt = $this->connection->prepare('SELECT ' . 'p.id id, ' . 'e.name name, ' . 'e.price price, ' . 'p.image image, ' . 'p.product_group pgroup, ' . 'p.viewable viewable, ' . 'DATE_FORMAT(e.edit, "%H") hour, ' . 'DATE_FORMAT(e.edit, "%i") mins, ' . 'DATE_FORMAT(e.edit, "%s") secs, ' . 'DATE_FORMAT(e.edit, "%d") day, ' . 'DATE_FORMAT(e.edit, "%c") month, ' . 'DATE_FORMAT(e.edit, "%Y") year ' . 'FROM ' . self::PRODUCTS . ' p ' . 'LEFT JOIN ' . self::PRODUCTS_EDITS . ' e ' . 'ON p.last_edit = e.id ' . 'LEFT JOIN ' . self::FORMULAS_CHOICES_PRODUCTS . ' c ' . 'ON c.product = p.id ' . 'WHERE c.choice = :id');
    ...
    {
        $stmt = $this->connection->prepare('SELECT ' . 'ID id, ' . 'Email email, ' . 'Pseudo alias, ' . 'Nom fname, ' . 'Prenom sname, ' . 'adherent member, ' . 'Credit balance, ' . 'Annee regyear ' . 'FROM ' . self::USERS . ' u ' . 'WHERE u.SU = 0 ' . 'AND (u.Pseudo LIKE :expression ' . 'OR u.Nom LIKE :expression ' . 'OR u.Prenom LIKE :expression) ' . 'ORDER BY u.Prenom');
